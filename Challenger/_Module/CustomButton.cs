...
namespace ChallengerMod.CustomButton
{
    ...
    {
        public static List<CustomButton> bttn = new List<CustomButton>();
        ...
        {
            bttn.RemoveAll(item => item.actionButton == null);
        ...
        public static void ResetAllCooldowns()
        {
            for (int i = 0; i < bttn.Count; i++)
            {
                try
                {
                    bttn[i].Timer = bttn[i].StartTimer + 0f;
                    bttn[i].isEffectActive = false;
                    bttn[i].Update();
                }
                catch (NullReferenceException)
                {
