...
namespace ChallengerMod.CustomButton
{
    ...
    static class HudManagerStartPatch
    {
        ...
        public static void UpdateCustomButtonCooldowns()
        {
            ...
            {
                if (ChallengerMod.Fix.BlockUtilitiesPatches.adminBool == true || task.TaskType == TaskTypes.FixComms)
                {
                        ChallengerMod.CustomButton.HudManagerStartPatch.AdminButton.Sprite = admin0;
                        ChallengerMod.CustomButton.HudManagerStartPatch.AdminButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }
                else
                {
                        ChallengerMod.CustomButton.HudManagerStartPatch.AdminButton.Sprite = admin1;
                        ChallengerMod.CustomButton.HudManagerStartPatch.AdminButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }


                if (ChallengerMod.Fix.BlockUtilitiesPatches.vitalsBool == true || task.TaskType == TaskTypes.FixComms)
                {
                        ChallengerMod.CustomButton.HudManagerStartPatch.VitalButton.Sprite = Vitale0;
                        ChallengerMod.CustomButton.HudManagerStartPatch.VitalButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }
                else
                {
                        ChallengerMod.CustomButton.HudManagerStartPatch.VitalButton.Sprite = Vitale1;
                        ChallengerMod.CustomButton.HudManagerStartPatch.VitalButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }

                if (ChallengerMod.Fix.BlockUtilitiesPatches.camsBool == true || task.TaskType == TaskTypes.FixComms)
                {
                        ChallengerMod.CustomButton.HudManagerStartPatch.CamButton.Sprite = cam0;
                        ChallengerMod.CustomButton.HudManagerStartPatch.CamButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }
                else
                {
                        ChallengerMod.CustomButton.HudManagerStartPatch.CamButton.Sprite = cam1;
                        ChallengerMod.CustomButton.HudManagerStartPatch.CamButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }
                ...
                {
                    ChallengerMod.CustomButton.HudManagerStartPatch.BuzzButton.Sprite = buzz0;
                    ChallengerMod.CustomButton.HudManagerStartPatch.BuzzButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);

                }
                else
                {
                    ChallengerMod.CustomButton.HudManagerStartPatch.BuzzButton.Sprite = buzz1;
                    ChallengerMod.CustomButton.HudManagerStartPatch.BuzzButton.actionButton.transform.localScale = new Vector3(0.5f, 0.5f, 0.5f);
                }
        ...
        public static void Postfix(HudManager __instance)
        {
            ...
            NuclearTimerButton = new CustomButton(
                ...
                () => {
                    ...
                    else if (NuclearTimerText != null && NuclearMap == true && NuclearTimer <= 0 && NuclearLastTimer > 0)
                    {
                       if (SafeTimer <= 0)
                        {
                            NuclearTimerText.text = "" + timerLN;
                            NuclearTimerText.color = RedColor;
                            NuclearTimerButton.Sprite = NuclearButtonSprite0;
                        }
                       else
                        {
                            NuclearTimerText.text = "" + timerLN;
                            NuclearTimerText.color = RedColor;
                            NuclearTimerButton.Sprite = NuclearButtonSprite1;
                        }
            ...
            SetUI = new CustomButton(
                () => {
                    if (ChallengerMod.Challenger.UICustom)
                    {
                        ChallengerMod.Challenger.UICustom = false;
                    }
                    else
                    {
                        ChallengerMod.Challenger.UICustom = true;
                    }
                    //use
            ...
            AdminButton = new CustomButton(
                ...
                () => {
                    if (AdminText != null && ChallengerOS.Utils.Option.CustomOptionHolder.AdminTimeOn.getSelection() != 0) AdminText.text = "" + timerA;
                    foreach (PlayerTask task in PlayerControl.LocalPlayer.myTasks)
                    {
                        if (ChallengerMod.Fix.BlockUtilitiesPatches.adminBool == true || task.TaskType == TaskTypes.FixComms || timerA <= 0)
                        {
                            AdminButton.actionButton.OverrideText("ADMIN");
                            AdminText.color = RedColor;
                            AdminButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.RedColor);

                        }
                        else
                        {
                            AdminButton.actionButton.OverrideText("ADMIN");
                            AdminText.color = GreenColor;
                            AdminButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.GreenColor);
                        }
                    }
                    return true; },
                () => { },
            ...
            VitalButton = new CustomButton(
                ...
                () => { return PlayerControl.LocalPlayer != null && ChallengerMod.Challenger.UICustom; },
                () => {
                    if (VitalText != null && ChallengerOS.Utils.Option.CustomOptionHolder.VitalTimeOn.getSelection() != 0) VitalText.text = "" + timerV;
                    foreach (PlayerTask task in PlayerControl.LocalPlayer.myTasks)
                    {
                        if (ChallengerMod.Fix.BlockUtilitiesPatches.vitalsBool == true || task.TaskType == TaskTypes.FixComms || timerV <= 0)
                        { 
                            VitalText.color = RedColor;
                            VitalButton.actionButton.OverrideText("VITAL");
                            VitalButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.RedColor);

                        }
                        else
                        { 
                            VitalText.color = GreenColor;
                            VitalButton.actionButton.OverrideText("VITAL");
                            VitalButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.GreenColor);
            ...
            CamButton = new CustomButton(
                ...
                () => { return PlayerControl.LocalPlayer != null && ChallengerMod.Challenger.UICustom; },
                () => {
                    if (CamText != null && ChallengerOS.Utils.Option.CustomOptionHolder.CamTimeOn.getSelection() != 0) CamText.text = "" + timerC;
                    foreach (PlayerTask task in PlayerControl.LocalPlayer.myTasks)
                    {
                        if (ChallengerMod.Fix.BlockUtilitiesPatches.camsBool == true || task.TaskType == TaskTypes.FixComms || timerC <= 0)
                        {
                            CamText.color = RedColor;
                            CamButton.actionButton.OverrideText("CAMERA");
                            CamButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.RedColor);
                        }
                        else
                        { 
                            CamText.color = GreenColor;
                            CamButton.actionButton.OverrideText("CAMERA");
                            CamButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.GreenColor);
                        }
                    }
            ...
            BuzzButton = new CustomButton(
                ...
                () => { return PlayerControl.LocalPlayer != null && ChallengerMod.Challenger.UICustom; },
                () => {
                    if (BuzzText != null) BuzzText.text = "" + PlayerControl.LocalPlayer.RemainingEmergencies;
                    foreach (PlayerTask task in PlayerControl.LocalPlayer.myTasks)
                    {
                        ...
                        { 
                            ...
                            BuzzButton.actionButton.OverrideText("BUZZ");
                            BuzzButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.RedColor);
                        }
                        else
                        { 
                            BuzzText.color = GreenColor;
                            BuzzButton.actionButton.OverrideText("BUZZ");
                            BuzzButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.GreenColor);
                        }
                    }
            ...
            SentinelVentButton = new CustomButton(
                ...
                () => {
                    if (VentText != null)
                    {
                        if (ChallengerOS.Utils.Option.CustomOptionHolder.ScanAbility.getSelection() != 1)
                        {
                            if (Sentinel.Scan && Sentinel.ScanPLayerInVent)
                            {
                                SentinelVentButton.Sprite = SenVon;
                                VentText.text = "-";
                                VentText.color = nocolor;
                            }
                            else
                            {
                                SentinelVentButton.Sprite = SenVoff;
                                VentText.text = "-";
                                VentText.color = nocolor;
                            }
                        }
                        ...
                        {
                            SentinelVentButton.Sprite = SenVnull;
                            VentText.text = "-";
                            VentText.color = nocolor;
                        }

                    }
                    SentinelVentButton.actionButton.OverrideText("VENT");
                    SentinelVentButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.SentinelColor);
                    return true;
                },
                () => { },
                SenV,
            ...
            SentinelBodyButton = new CustomButton(
                ...
                () => {
                    if (BodyText != null)
                    {
                        if (ChallengerOS.Utils.Option.CustomOptionHolder.ScanAbility.getSelection() != 0)
                        {
                            if (Sentinel.Scan && Sentinel.ScanPlayerDie > 0 && PlayerControl.LocalPlayer == Sentinel.Role)
                            {
                                SentinelBodyButton.Sprite = SenDBon;
                                BodyText.text = "" + Sentinel.ScanPlayerDie;
                                BodyText.color = PurpleColor;
                            }
                            else if (Sentinel.Scan && CopyCat.ScanPlayerDie > 0 && PlayerControl.LocalPlayer == CopyCat.Role)
                            {
                                SentinelBodyButton.Sprite = SenDBon;
                                BodyText.text = "" + CopyCat.ScanPlayerDie;
                                BodyText.color = PurpleColor;
                            }
                            else
                            {
                                SentinelBodyButton.Sprite = SenDBoff;
                                BodyText.text = "-";
                                BodyText.color = nocolor;
                            }
                        }
                        else
                        {
                            SentinelBodyButton.Sprite = SenDBnull;
                            BodyText.text = "-";
                            BodyText.color = nocolor;
                        }

                    }
                    SentinelBodyButton.actionButton.OverrideText(BTT_ScanBody);
                    SentinelBodyButton.actionButton.buttonLabelText.SetOutlineColor(ChallengerMod.ColorTable.SentinelColor);
                    return true;
                },
                () => { },
                SenDB,
            ...
            EaterBarAbilityButton = new CustomButton(
                ...
                () => {
                    ...
                    if (Eater.DistBody >= 10)
                        EaterBarAbilityButton.Sprite = E10;
                    if (Eater.DistBody == 9)
                        EaterBarAbilityButton.Sprite = E9;
                    if (Eater.DistBody == 8)
                        EaterBarAbilityButton.Sprite = E8;
                    if (Eater.DistBody == 7)
                        EaterBarAbilityButton.Sprite = E7;
                    if (Eater.DistBody == 6)
                        EaterBarAbilityButton.Sprite = E6;
                    if (Eater.DistBody == 5)
                        EaterBarAbilityButton.Sprite = E5;
                    if (Eater.DistBody == 4)
                        EaterBarAbilityButton.Sprite = E4;
                    if (Eater.DistBody == 3)
                        EaterBarAbilityButton.Sprite = E3;
                    if (Eater.DistBody == 2)
                        EaterBarAbilityButton.Sprite = E2;
                    if (Eater.DistBody == 1)
                        EaterBarAbilityButton.Sprite = E1;
                    if (Eater.DistBody == 0)
                        EaterBarAbilityButton.Sprite = E0;
            ...
            CursedBarAbilityButton = new CustomButton(
                ...
                () => {
                    ...
                    CursedBarAbilityButton.actionButton.transform.localScale = new Vector3(1f, 1f, 1f);


                    if (Cursed.CursePercent == 0)
                        CursedBarAbilityButton.Sprite = CB0;
                    else if (Cursed.CursePercent < 5)
                        CursedBarAbilityButton.Sprite = CB0;
                    else if (Cursed.CursePercent < 10)
                        CursedBarAbilityButton.Sprite = CB1;
                    else if (Cursed.CursePercent < 15)
                        CursedBarAbilityButton.Sprite = CB2;
                    else if (Cursed.CursePercent < 20)
                        CursedBarAbilityButton.Sprite = CB3;
                    else if (Cursed.CursePercent < 25)
                        CursedBarAbilityButton.Sprite = CB4;
                    else if (Cursed.CursePercent < 30)
                        CursedBarAbilityButton.Sprite = CB5;
                    else if (Cursed.CursePercent < 35)
                        CursedBarAbilityButton.Sprite = CB6;
                    else if (Cursed.CursePercent < 40)
                        CursedBarAbilityButton.Sprite = CB7;
                    else if (Cursed.CursePercent < 45)
                        CursedBarAbilityButton.Sprite = CB8;
                    else if (Cursed.CursePercent < 50)
                        CursedBarAbilityButton.Sprite = CB9;
                    else if (Cursed.CursePercent < 55)
                        CursedBarAbilityButton.Sprite = CB10;
                    else if (Cursed.CursePercent < 60)
                        CursedBarAbilityButton.Sprite = CB11;
                    else if (Cursed.CursePercent < 65)
                        CursedBarAbilityButton.Sprite = CB12;
                    else if (Cursed.CursePercent < 70)
                        CursedBarAbilityButton.Sprite = CB13;
                    else if (Cursed.CursePercent < 75)
                        CursedBarAbilityButton.Sprite = CB14;
                    else if (Cursed.CursePercent < 80)
                        CursedBarAbilityButton.Sprite = CB15;
                    else if (Cursed.CursePercent < 85)
                        CursedBarAbilityButton.Sprite = CB16;
                    else if (Cursed.CursePercent < 90)
                        CursedBarAbilityButton.Sprite = CB17;
                    else if (Cursed.CursePercent < 95)
                        CursedBarAbilityButton.Sprite = CB18;
                    else if (Cursed.CursePercent < 100)
                        CursedBarAbilityButton.Sprite = CB19;
                    else if (Cursed.CursePercent >= 100)
                        CursedBarAbilityButton.Sprite = CB20;
            ...
            // Cursed
           /* CursedUpAbilityButton = new CustomButton(
                () => {
                    MurderAttemptResult murderAttemptResult = Helpers.checkMurderAttempt(Cursed.Role, Cursed.currentTarget);
                },
                () => { return Cursed.Role != null && Cursed.Role == PlayerControl.LocalPlayer && !PlayerControl.LocalPlayer.Data.IsDead; },
                () => {

                    if (Cursed.NoCurse == true || Cursed.CurseStart == false)
                    {
                        CursedUpAbilityButton.Sprite = AR_0;
                    }
                    else
                    {
                        if (Cursed.currentTarget == null)
                        {
                            CursedUpAbilityButton.Sprite = AR_UP;
                        }
                        else
                        {
                            CursedUpAbilityButton.Sprite = AR_DOWN;
                        }
                    }
            ...
            MysticAbilityButton = new CustomButton(
                () => {
                    ...
                    //HarmonyMain.Instance.RpcControlPlayer();
            ...
            MorphlingMorphAbilityButton = new CustomButton(
                () => {
                    ...
                    ChallengerOS.Utils.Helpers.showFlash(new Color(255f / 255f, 50f / 255f, 255f / 255f), 1f);
            ...
            Sorcerer2AbilityButton = new CustomButton(
                () => {
                    ...
                    if (Sorcerer.Scan1 == null)
                    {
                        ...
                        ChallengerOS.Utils.Helpers.showFlash(new Color(150f / 255f, 100f / 255f, 50f / 255f), 0.5f);
                        Sorcerer1AbilityButton.Timer = 0.5f;
                        Sorcerer2AbilityButton.Timer = 0.5f;
                        Sorcerer3AbilityButton.Timer = 0.5f;
                        Sorcerer4AbilityButton.Timer = 0.5f;
            ...
            Sorcerer3AbilityButton = new CustomButton(
                () => {
                    ...
                    ChallengerOS.Utils.Helpers.showFlash(new Color(150f / 255f, 150f / 255f, 150f / 255f), 0.5f);
                    Sorcerer1AbilityButton.Timer = 0.5f;
                    Sorcerer2AbilityButton.Timer = 0.5f;
                    Sorcerer3AbilityButton.Timer = 0.5f;
                    Sorcerer4AbilityButton.Timer = 0.5f;
